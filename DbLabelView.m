...
float dBlabelView_isDevice(void)
{
    /*
     iPhone 4 : 960 * 640
     iPhone 5 :  1136 * 640
...
-(void)setdBLabel : (CGPoint) xyPoint flag : (int) flag
{
    NSString *y_label[9] = {@"100",@"  90",@"  80",@"  70",@"  60",@"  50",@"  40",@"  30",@"  20"};
    if(flag == 9){
        displayDb = [[UILabel alloc] initWithFrame:CGRectMake(0,-7, 30, 20)];
...
-(void)drawRect:(CGRect)rect{
    ...
    isDeviceValue = dBlabelView_isDevice();
...
//    printf("LabelView commonInit self width : %f    height : %f\n\n",self.frame.size.width,self.frame.size.height);
    ...
    for(int i = 0; i< 10; i++){
        if(i == 9){
            [self setdBLabel:startPoint flag:i];
